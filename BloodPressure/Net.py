...
class BaseBlock(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(self.input_channels, self.output_channels//2, kernel_size = 1, padding = 0),
            nn.BatchNorm1d(self.output_channels//2),
            nn.ReLU(inplace = True),
            nn.Conv1d(self.output_channels//2, self.output_channels//2, kernel_size = 5, padding = 2),
            nn.BatchNorm1d(self.output_channels//2),
            nn.ReLU(inplace = True),
